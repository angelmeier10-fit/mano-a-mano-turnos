...
export function applyClientDiscount(price, discount) {
  const base = price || 0;
  ...
  const discountAmount = discount.discountType === "percent"
    ? Math.round(base * (discount.discountValue / 100))
    : Math.min(base, discount.discountValue);
  return { finalPrice: Math.max(0, base - discountAmount), discountAmount };
}
